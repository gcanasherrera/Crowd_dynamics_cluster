function [time, xsols, vsols, Mass, l] = CrowdsG(n,t,Case,mcase);
tic
r = 1/4;
% masses
if mcase == 1
...
% ode function
    function dwdt = eqn(~,w)
        ...
        %FP(2:n)= (P/2)*((2/pi)*atan(100*Del(2:n))+1).*((sign(V(1:n-1)-V(2:n))).^2).*((sign(V(1:n-1)-V(2:n))+1)/2)...
        %    -(P/2)*(2/pi)*atan(100*(Del(2:n)-Delta0)+1).*((sign(V(1:n-1)-V(2:n))).^2).*((sign(V(1:n-1)-V(2:n))-1)/2).^2 ...
        %    -(P/2)*(2/pi)*atan(100*(Del(2:n)-Delta0)+1).*((sign(V(1:n-1)-V(2:n))-1)).*((sign(V(1:n-1)-V(2:n))+1));
                FP(2:n) = (1/10)*((sign(V(1:n-1)-V(2:n))).^2).*...
